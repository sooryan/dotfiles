...
Clone(){
    cd ~/Clones/Github; git clone "$1" || exit
...
vcreate(){
    virtualenv --system-site-packages ~/.virtualenvs/"$1"
}
...
vcreate3(){
    virtualenv -p python3 --system-site-packages ~/.virtualenvs/"$1"
}
...
clock(){
        while sleep 1;do tput sc;tput cup 0 $(($(tput cols)-10));echo -e "\e[31m$(date +%T)\e[39m";tput rc;done &
}
